...
"""
创建一个支持多线程、异步并发和重连接机制的 PyMySQL MySQL 类：
多线程支持：我们将确保每个线程都有自己的数据库连接，以避免线程安全问题。
为此，我们可以使用 Python 的 threading.local() 来为每个线程维护独立的连接。
异步并发：虽然 PyMySQL 本身不直接支持异步操作，我们可以结合 asyncio 和 aiomysql 库来实现异步功能。
aiomysql 是 PyMySQL 的异步版本，与 asyncio 兼容。

重连接机制：我们将实现一个简单的重连接逻辑，在遇到连接丢失或超时的情况下自动重新连接。

多进程环境：在多进程环境中，每个进程应该有自己的数据库连接池，以确保进程间的安全性和性能。我们可以通过配置连接池大小和连接参数来优化性能。

实现方案
我们将创建一个名为 MySQLClient 的类，该类将封装 PyMySQL 和 aiomysql 的功能，并提供同步和异步两种接口。
此外，我们还将添加一个重连接机制，以确保在连接丢失时能够自动恢复。
...
日志记录：
使用 logging 模块记录连接创建、重连等关键操作的日志，方便调试和监控。
使用示例
...
注意事项
连接池大小：根据你的应用负载和数据库服务器的配置，合理调整 maxsize 参数，以确保最佳性能。
事务管理：如果你需要更复杂的事务管理（如多条语句的原子性），可以在 execute_write_sync 和 execute_write_async 方法中加入事务控制。
错误处理：在实际应用中，建议进一步完善错误处理逻辑，特别是在网络不稳定或数据库服务器出现故障的情况下。
...
"""
1. 单个实例化 + 线程本地连接
你可以只实例化一次 MySQLClient 类，并通过 threading.local() 为每个线程分配独立的连接。这种方式下，你不需要为每个线程单独实例化 MySQLClient，但每个线程会有自己的连接。
"""
...
"""
2. 每个线程独立实例化如果你希望每个线程都有自己的 MySQLClient 实例，你可以在每个线程中单独实例化 MySQLClient。这种方式可以进一步隔离线程之间的资源，但可能会增加内存开销。
"""
...
"""
推荐做法
推荐使用第一种方式，即单个实例化 + 线程本地连接。这种方式既保持了代码的简洁性，又确保了线程安全和性能优化。具体来说：
...
每个线程独立实例化 虽然也可以工作，但通常没有必要，除非你有特殊的需求或非常复杂的场景。
"""