...
class NET_WHITE_LIST_AUTHORITY_LIST(Structure):
    """
    允许名单权限列表;authority list of white list
    """
    _fields_ = [
...
class NET_TRAFFICCAR_WHITE_LIST(Structure):
    """
    允许名单信息;white list information
    """
    _fields_ = [
        ('bTrustCar', c_int),         # 车牌是否属于允许名单;true: the car is trust car,false:the car is not trust car
        ('stuBeginTime', NET_TIME),   # 允许名单起始时间;begin time of white list
        ('stuCancelTime', NET_TIME),  # 允许名单过期时间;cancel time of white list
        ('stuAuthorityList', NET_WHITE_LIST_AUTHORITY_LIST),  # 允许名单权限列表;authority list of white list
        ('bReserved', c_ubyte*32)     # 保留字节;Reserved
    ]

class NET_TRAFFICCAR_BLACK_LIST(Structure):
    """
    禁止名单信息;Blacklist information
    """
    _fields_ = [
        ('bEnable', c_int),          # 禁止名单信息;Enable blacklist
        ('bIsBlackCar', c_int),      # 车牌是否属于禁止名单;Whether is the plate on the blacklist or not
...
class DEV_EVENT_TRAFFIC_TRAFFICCAR_INFO(Structure):
    ...
    _fields_ = [
        ...
        ('nTrafficBlackListID', c_int),     # 关联禁止名单数据库记录默认主键ID, 0,无效；> 0,禁止名单数据记录;Link black list data recorddefualt main keyID, 0, invalid, > 0, black list data record
        ...
        ('stuWhiteList', NET_TRAFFICCAR_WHITE_LIST),    # 允许名单信息;white list information
        ...
        ('stuBlackList',NET_TRAFFICCAR_BLACK_LIST),     # 禁止名单信息;Blacklist information
...
class NET_FACE_ATTRIBUTE_EX(Structure):
    ...
    _fields_ =[
        ...
        ('szReserved', c_char * 4),         # 保留; reserved
...
class DEV_EVENT_FACEDETECT_INFO(Structure):
    ...
    _fields_ = [
        ...
        ('szReserved1',  c_char * 4),                   # 保留; reserved 
...
class SCENE_IMAGE_INFO_EX(Structure):
    ...
    _fields_ = [
        ...
        ('byReserved', c_char*512),      # 预留字节;Reserved
    ]
